...
def x1_f(i):
    ...
    A = np.ones(i).reshape(i, 1)
    return np.concatenate([alpha.flatten(), c_bar.flatten(), A.flatten()])
...
# with pd.ExcelWriter('USA_inputs.xlsx', engine='xlsxwriter') as writer:
#     pd.DataFrame(beta).to_excel(writer, sheet_name='beta')
#     pd.DataFrame(sigma).to_excel(writer, sheet_name='sigma')
#     pd.DataFrame(share_lab_d_dict).to_excel(writer, sheet_name='labour_share')
#     pd.DataFrame(A_dict).to_excel(writer, sheet_name='TFP')
...
def equilibrium(x1, beta, sigma, L, A = A, i = i):
    ...
    c_bar = x1[i:8].reshape(i, 1); alpha = x1[:i].reshape(i, 1)
...
def obj1(x1, beta, sigma, L, share_cons_d, share_lab_d_dict, A_dict):
    ...
    x1[:i] = x1[:i] / np.sum(x1[:i])  # normaliza alpha
    
    x1[6] = x1[7] # c_barras iguais 
    ...
    def calcular_obj_por_ano(year):
        ...
        ## Labor Share
        P1 = np.sum(np.power(np.divide(share_lab_m - share_lab_d, share_lab_d), 2)) 
        
        ## Consumption Share
        if year == 1950:
            P2 = 0
        else:
            P2 = np.sum(np.power(np.divide(share_cons_m - share_cons_d, share_cons_d), 2))
                
        obj = P1 + P2

        ## Constraints
        if np.any(result[5] < 0) or np.any(result[8] < 0):
            penalty = 1e6 * (np.sum(np.abs(result[5][result[5] < 0])) +
                             np.sum(np.abs(result[8][result[8] < 0])))
            obj += penalty
...
Bd = ((0.01, 0.6), )*i + ( (0.00, 0.01), )*i   + ( (0.1, 10.0), )*i  

my_iter = 500

cc=0

# array([0.0374, 0.0099, 0.2625, 0.6902, 0.0023, 0.0028, 0.    , 0.0061])
...
for year in years:
    ...
    for ii, my_groups in enumerate(dt.group_names):
        res.append(
            ...
            'share_cons_m': share_cons_m[ii].item(),
            'share_cons_d': share_cons_d[ii].item(),          
            }
        )
