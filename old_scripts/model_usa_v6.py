...
# I will try to calibrate c_bar and tfp using share of gross output and labor
...
share_go_d = dt.compute_dataset(code = 'USA', ano = year)[2]
...
def callback(x1):    
    global cc
    cc += 1
    fobj = obj1(x1, beta, sigma, L, share_cons_d, share_lab_d, share_go_d)
...
### Objetive function
def obj1(x1, beta, sigma, L, share_cons_d, share_lab_d, share_go_d):    
    ...
    obj = np.sum(np.power(np.divide(share_lab_m - share_lab_d, share_lab_d), 2)) + \
          np.sum(np.power(np.divide(share_go_m - share_go_d, share_go_d), 2)) 
...
# 3 labsh
# 3 gosh
# tt = share_cons_d[:3]#[1:4]
# np.append(tt, 1-tt.sum())
...
sol = differential_evolution(
        obj1,
        bounds=Bd,
        args=(beta, sigma, L, share_cons_d, share_lab_d, share_go_d),
